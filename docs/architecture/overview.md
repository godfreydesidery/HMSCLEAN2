...
| Security | **Spring Security 6 + JWT** | Custom auth0 filters, hardcoded `"<REDACTED>"` | Preserve User→Role→Privilege RBAC + `privileges` claim + 177 `@PreAuthorize`; key in secrets store + rotation (ADR-0006) |
