...
/**
 * Issues unique, monotonic document numbers (CR-09; ADR-0009 §5/§6).
 *
 ...
 * mechanism computed the next number before insert, so two concurrent writers could read the same
 * {@code MAX(id)} and produce duplicate numbers, tripping the {@code UNIQUE(no)} constraint on the
 * second insert (04-extract-creditnote-numbering-refund.md §4). The sequence-backed implementation
 * is collision-free; the emitted string is byte-identical in shape to the legacy one.
 *
 ...
 *
 * <p>Lives in the shared kernel (inc-08 Q11 ratified) so pharmacy, inventory, billing and any other
 * context can allocate numbers without a cross-module dependency on a domain repository.
 */
